...
F(2:end-1,2:end-1).*v(2:end-1,2:end-1) ...
              - (g/(2*dx)).*(H(3:end,2:end-1)-H(1:end-2,2:end-1));

	# Compute the accelerations
	u_accel = F[1:nx-2,1:ny-2]*v[1:nx-2,1:ny-2] - (g/(2*dx))*(H[2:nx,2:ny-1]-H[1:nx-2,2:ny-1]);
	v_accel = -F[2:nx-1,2:ny-1]*u[2:nx-1,2:ny-1] - (g/(2*dy))*(H[2:nx-1,3:ny]-H[2:nx-1,1:ny-2]);
