...
#	sys.exit();
	...
	#np.save('u_complex.npy',u[:,:,ts]);
	#np.save('v_complex.npy',v[:,:,ts]);
	#np.save('h_complex.npy',h[:,:,ts]);

	#plotting.solutionPlotsPhase(x_grid,y_grid,u,v,h,ts,FORCE,BG,Fpos,N);
	#plotting.solutionPlotsAmp(x_grid,y_grid,u,v,h,ts,FORCE,BG,Fpos,N);
	...
	if doEnergy:
		...
		tii = 10
		KE, KE_tot = energy.KE(u_full[:,:,tii],v[:,:,tii],h_full[:,:,tii],x_nd,y_nd,dx_nd,dy_nd,N);
		PE, PE_tot = energy.PE(h_full[:,:,tii],Ro,x_nd,y_nd,dx_nd,dy_nd,N);
		E = KE + PE;
		E_tot = KE_tot + PE_tot;
