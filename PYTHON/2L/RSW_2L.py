...
# Forcing
F1_nd, F2_nd, F3_nd, F4_nd, F5_nd, F6_nd, Ftilde1_nd, Ftilde2_nd, Ftilde3_nd, Ftilde4_nd, Ftilde5_nd, Ftilde6_nd = forcing_2L.Forcing(x,y,K,y0,r0,N,FORCE1,FORCE2,AmpF,g,f,f0,U,L,rho1_nd,rho2_nd,dx,dy);
...
save = False;
if save:
	# Saving solutions (pre-IFFT) so that they can be better visualised in MATLAB
	import scipy.io as sio
	sio.savemat('/home/mike/Documents/GulfStream/Code/DATA/2L/' + str(FORCE1) + '/' + str(BG1) + '/u_nd_' + str(Fpos) + str(N),{'u_nd':u1_nd});
	sio.savemat('/home/mike/Documents/GulfStream/Code/DATA/2L/' + str(FORCE1) + '/' + str(BG1) +  '/v_nd_' + str(Fpos) + str(N),{'v_nd':v1_nd});
	sio.savemat('/home/mike/Documents/GulfStream/Code/DATA/2L/' + str(FORCE1) + '/' + str(BG1) +  '/eta_nd_' + str(Fpos) + str(N),{'eta_nd':eta0_nd});

	# Save solutions again in npy format, so they can be operated on for diagnostic/analytic purposes
	np.save('/home/mike/Documents/GulfStream/Code/DATA/2L/' + str(FORCE1) + '/' + str(BG1) +  '/u_nd_' + str(Fpos) + str(N),u1_nd);
	np.save('/home/mike/Documents/GulfStream/Code/DATA/2L/' + str(FORCE1) + '/' + str(BG1) +  '/v_nd_' + str(Fpos) + str(N),v1_nd);
	np.save('/home/mike/Documents/GulfStream/Code/DATA/2L/' + str(FORCE1) + '/' + str(BG1) +  '/eta_nd_' + str(Fpos) + str(N),eta0_nd);
